...
def send_payment_mail(
    ...
    if result == PaymentResult.Succeeded:
        ...
        msg_plain = render_to_string(
            "invoice_failed_template.txt",
            {"checkout_url": invoice_url if invoice_url else ""},
        )
        msg_html = render_to_string(
            "invoice_failed_template.html",
            {"checkout_url": invoice_url if invoice_url else ""},
        )
    else:
        raise NotImplementedError(
            "This payment result is not implemented in 'send_payment_mail'"
        )
